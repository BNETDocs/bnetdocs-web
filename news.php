<?
	...
	$mode = $_GET['mode'];
	$newsicon = strtoupper($_POST['pictureselector']);
	$title = $_POST['subject'];
	$content = $_POST['text'];
	...
	if(!$_POST || $_POST['operation']){
		...
		if($mode == 'edit'){
			...
			if($userid != $pid){
				...
				blockhack(); # Jailout2000: Undefined function, need to make it a function.
			}
			...
			while($row = mysql_fetch_array($newsarray)){
				...
				$dtstamp = $row['dtstamp'];
				$postdate = date('F jS, Y g:i A T', strtotime($dtstamp));
				$icon = strtolower($row['topictype']);
				$title = $row['subject'];
				#$content = html2bb(bb2html($row['content']));
				$content = BBCode($row['content']);
				$topictype = 'images/'.strtolower($row['topictype']).'icon.png';
				$sqlquery2 = 'SELECT * FROM comments WHERE catid=1 AND subid='.$nid.' ORDER BY id ASC';
				...
				if($rank > 1 && $rank != 6){
					#Assign Rank
					if($rank == 3) $rankname = '  <font class="tiny">(EDITOR)</font> ';
					if($rank == 4 || $rank == 5) $rankname = ' <font class="tiny">(ADMIN)</font> ';
					if($userid){
						if($userid == $author) $edit = ' <font class="hlbutton">&nbsp;Edit </font>&nbsp;|';
						if($rank > 2) $delete = ' <font class="hlbutton">&nbsp;Delete </font>&nbsp;|';
					}
				}
				if(!$rnid){
					$commentslink = '<a class="hlbutton" href="?op=news&amp;nid='.$nid.'#comments">&nbsp;'.$numrows.' Comments </a>&nbsp;|';
				} else {
					$commentslink = $numrows.' Comments |';
				}
				DisplayDialogBox($title, $content, false, '<span style="float: left; padding-left: 5px;">'.$postdate.'</span>Posted by '.whoisid($author).$rankname.' | '.$commentslink.$edit.$delete, false, $topictype);
				if($rnid) { # If viewing a specific news post, include comments.
					$catid = 1;	#News category
					$subid = $nid;
